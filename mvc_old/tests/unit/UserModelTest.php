...
class UserModelTest extends \PHPUnit\Framework\TestCase {
	...
	public function all_is_working() {
		...
		$getByUsernameAndPassword = UserModel::getByUsernameAndPassword($user['username'], $user['password']);
		$this->assertNotFalse($getByUsernameAndPassword);
		$this->assertEquals($getByUsernameAndPassword->last_name, $user['last_name']);
		$this->userAsserts($getByUsernameAndPassword, $create);
